const { Client, EmbedBuilder, PermissionFlagsBits, UserSelectMenuBuilder, ActionRowBuilder } = require("discord.js");
const {createTranscript} = require('discord-html-transcripts');
const TicketSetup = require('../../database/TicketSetupDB');
const TicketSchema = require('../../database/TicketDB');
        ...
        const data = await TicketSchema.findOne({GuildID: guild.id, ChannelID: channel.id})
            ...
            switch (customId) {
                case 'ticket-close':
                    ...
                    .setDescription(`Membro: <@${data.OwnerID}>\n Ticket: ${data.TicketID}\n Reivindicado: ${claimed}\n Moderador: ${data.ClaimedBy}\n Tempo: <t:${transcriptTimestamp}:R> (<t:${transcriptTimestamp}:F>)`)
                    const closingTicket = new EmbedBuilder().setTitle('O ticket está sendo fechado no momento...').setDescription('ticket será fechado em 5 segundos.').setColor('Red')
                ...
                case 'ticket-unlock':
                    ...
                    executeEmbed.setDescription('Ticket foi desbloqueado com sucesso.');
                ...
                case 'ticket-claim':
                    if ((!member.permissions.has(ManageChannels)) & (!member.roles.cache.has(docs.Handlers))) return interaction.reply({embeds: [nopermissionsEmbed], ephemeral: true}).catch(error => {return});
                    alreadyEmbed.setDescription('Ticket já foi reivindicado por' + ' <@' + data.ClaimedBy + '>.');
                    ...
                    executeEmbed.setDescription('Ticket foi reivindicado com sucesso por' + ' <@' + member.id + '>.');
                    interaction.deferUpdate().catch(error => {return});
